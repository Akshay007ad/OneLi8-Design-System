...
const FOUNDATIONS = {
  ...
  naming: {
    grammar:
      'Sanskrit grammar, not a CSS convention. Two rules follow from it and together ' +
      'they explain every name in the system. First, a compound that names ONE thing is ' +
      'written as one word: actionprimary is a single role rather than action plus ' +
      'primary, and cutedge is one feature of the Gem material, so action-primary would ' +
      'suggest two things where there is one. Second, a qualifier stays SEPARATE from ' +
      'what it qualifies: in --ol8-material-control-gem--primary-hover-start, hover is a ' +
      'state applied to start, two grammatical roles, so they stay apart. That is why ' +
      'disabledcontent is joined while hover-start is not. Hyphens separate levels of the ' +
      'hierarchy, never words inside a level. Words stay readable: no abbreviations, no ' +
      'truncation, no btn where button will do.',
    authority:
      'Figma is the authority on the result. Every variable there carries an explicit ' +
      'codeSyntax, copied verbatim into packages/tokens/src/figma-code-syntax.txt. Read a ' +
      'name from that record rather than re-deriving it from the grammar, because the file ' +
      'is not perfectly regular between siblings and guessing is how code and design ' +
      'drifted apart before. The grammar is the rule; the record is the spelling.',
  },
