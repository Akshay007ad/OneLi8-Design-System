...
/* Both packages carry the registry, and both files say they are generated here.
   Only the react path was being written, so core's pool sat frozen at whatever
   it held when this path was repointed, and a new icon never reached it. The
   two files have the same shape, so both are written from the same pass. */
const OUTS = [
  join(ROOT, 'packages/react/src/foundations'),
  join(ROOT, 'packages/core/src/atoms/icon'),
];
